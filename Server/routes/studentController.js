...
module.exports = (app) => {
    app.get('/students/all', ensureAuthenticated, (req, res) => {
    ...
    app.delete('/students/delete/:id', ensureAuthenticated, checkIfAdmin, (req, res) => {
        studentService.deleteStudentById(req.params.id).then(data => {
            res.status(200).json({success: true, message: "התלמיד נמחק בהצלחה"});
        }).catch(err => {
    ...
    app.post('/students/new', ensureAuthenticated, checkIfAdmin, (req, res) => {
        studentService.addNewStudent(req.body).then(data => {
