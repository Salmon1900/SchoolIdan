...
module.exports = (app) => {
  ...
  app.get(
    "/classes/get/studentsCount/:classId",
    ensureAuthenticated,
    (req, res) => {
      classService.getStudentsInClass(req.params.classId).then((data) => {
  ...
  app.post("/classes/students/add/single", ensureAuthenticated, (req, res) => {
    classService
      .addStudentToClass(req.body.classId, req.body.studentId)
      .then((data) => {
        res
          .status(200)
          .json({ success: true, message: "התלמיד נוסף לכיתה בהצלחה" });
      })
      .catch((err) => {
  ...
  app.delete(
    "/classes/students/remove/single",
    ensureAuthenticated,
    (req, res) => {
      classService
        .removeStudentFromClass(req.body.classId, req.body.studentId)
        .then((data) => {
          res
            .status(200)
            .json({ success: true, message: "התלמיד הוסר מהכיתה בהצלחה" });
        })
        .catch((err) => {
          res.status(400).json({ success: false, message: err.message });
        });
  ...
  app.post("/classes/students/add/list", ensureAuthenticated, (req, res) => {
    classService
      .addStudentListToClass(req.body.classId, req.body.studentList)
      .then((data) => {
        res
          .status(200)
          .json({ success: true, message: "התלמידים נוספו לכיתה בהצלחה" });
      })
      .catch((err) => {
        res.status(400).json({ success: false, message: err.message });
      });
  ...
  app.put(
    "/classes/deactivate/:id",
    ensureAuthenticated,
    checkIfAdmin,
    (req, res) => {
      classService
        .deactivateClass(req.params.id)
        .then(() => {
          res
            .status(200)
            .json({ success: true, message: "הכיתה הושבתה בהצלחה" });
        })
        .catch((err) => {
          res.status(400).json({ success: false, message: err.message });
        });
  ...
  app.put(
    ...
    (req, res) => {
      classService
        .activateClass(req.params.id)
        .then(() => {
          res
            .status(200)
            .json({ success: true, message: "הכיתה הופעלה בהצלחה" });
        })
        .catch((err) => {
          res.status(400).json({ success: false, message: err.message });
        });
    ...
    (req, res) => {
      classService
        ...
        .then(() => {
          res
            .status(200)
            .json({ success: true, message: "הכיתה נמחקה בהצלחה" });
        })
        .catch((err) => {
          res.status(400).json({ success: false, message: err.message });
        });
