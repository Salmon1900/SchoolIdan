...
const newClassValid = async (classRec, teacher, subejct, teacherQualifications) => {
    let reason = "";

    let qualified = await Boolean(teacherQualifications.find(q => q.subject_id === classRec.subjectId))
    ...
    if((Number(classRec.schoolYear) < 1900 || Number(classRec.schoolYear > 2100) || Number.isNaN(Number(classRec.schoolYear)))){
        reason = reason.concat(`${labels.cls.wrongSchoolYear}\n`);
    }
    
    if((Number(classRec.birthYear) < 1900 || Number(classRec.birthYear > 2100) || Number.isNaN(Number(classRec.birthYear)))){
...
const addStudentListValid = async (classRec, studentIdList, allStudents) => {
    ...
    await studentIdList.forEach(async (studentId) => {
        let student = await allStudents.find(student => student.student_id === studentId)
        if(classRec && studentId.length !== 9){
            reason = reason.concat(`${labels.cls.idWrongLengthList} - ${studentId}`)
        } else if(!student){
            reason = reason.concat(`${labels.cls.studentDoesNotExistList} - ${studentId}`)
        } else if(classRec && student.birth_year !== classRec.student_birth_year){
